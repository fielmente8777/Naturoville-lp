import { introProps } from "@/@types/types";
import {
  LazyLoadedVideo,
  LinkButton,
  SectionHeadingDesc,
  SectionWithContainer,
} from "@/components";

const NewCelebrate: React.FC<introProps> = ({
  title,
  description,
  points,
  video,
  images,
  cta,
}) => {
  return (
    <SectionWithContainer sectionClassName="shadow-inner">
      <div className="grid grid-cols-1 lg:grid-cols-6 items-center gap-8">
        <div className="flex flex-col gap-6 lg:col-span-4">
          <SectionHeadingDesc title={title} titleClassName="max-lg:text-2xl" />
          {/* <div className="w-full aspect-[4/3] lg:hidden lg:col-span-4">
            <NewCelebrateSlider images={images} />
          </div> */}
          <div className="relative w-full aspect-4/6 rounded-sm lg:hidden">
            <LazyLoadedVideo
              src={video.src}
              poster={video.poster}
              loop
              muted
              autoPlay
              controls={false}
            />
          </div>
          {description.map((item, index) => (
            <p key={index} className="text-light md:text-lg">
              {item}
            </p>
          ))}

          <ul className="flex flex-col gap-4">
            {points.map((item, index) => (
              <li key={index} className="flex gap-4">
                <Foo />
                <span className="text-light md:text-lg">{item}</span>
              </li>
            ))}
          </ul>
          <LinkButton
            href={cta.href}
            label={cta.label}
            showIcon
            className="bg-primary w-fit text-white rounded-full!"
          />
        </div>
        <div className="w-full rounded-sm lg:block hidden lg:col-span-2">
        <div className="relative w-full aspect-4/6 max-w-3xl ml-auto">
          <LazyLoadedVideo
            src={video.src}
            poster={video.poster}
            loop
            muted
            autoPlay
            controls={false}
          />
          </div>
        </div>
        {/* <div className="w-full aspect-[4/3] lg:block hidden lg:col-span-4">
          <NewCelebrateSlider images={images} />
        </div> */}
      </div>
    </SectionWithContainer>
  );
};

export default NewCelebrate;

export const Foo = () => (
  <svg
    width={24}
    height={24}
    viewBox="0 0 24 24"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M8.24277 7.33992L1.86277 8.26492L1.74977 8.28792C1.57871 8.33333 1.42276 8.42333 1.29786 8.54872C1.17296 8.67412 1.08357 8.83041 1.03883 9.00165C0.994087 9.17289 0.995593 9.35293 1.04319 9.5234C1.09079 9.69386 1.18278 9.84864 1.30977 9.97192L5.93177 14.4709L4.84177 20.8259L4.82877 20.9359C4.81829 21.1128 4.85503 21.2894 4.93522 21.4474C5.0154 21.6055 5.13615 21.7394 5.2851 21.8354C5.43405 21.9315 5.60585 21.9862 5.78291 21.994C5.95997 22.0018 6.13593 21.9625 6.29277 21.8799L11.9988 18.8799L17.6918 21.8799L17.7918 21.9259C17.9568 21.9909 18.1362 22.0109 18.3115 21.9837C18.4868 21.9565 18.6517 21.8832 18.7894 21.7712C18.927 21.6592 19.0323 21.5127 19.0946 21.3466C19.1569 21.1804 19.1738 21.0008 19.1438 20.8259L18.0528 14.4709L22.6768 9.97092L22.7548 9.88592C22.8662 9.74869 22.9392 9.58438 22.9665 9.40972C22.9937 9.23506 22.9742 9.0563 22.9099 8.89165C22.8455 8.72701 22.7387 8.58235 22.6003 8.47244C22.4618 8.36252 22.2967 8.29126 22.1218 8.26592L15.7418 7.33992L12.8898 1.55992C12.8072 1.39246 12.6795 1.25144 12.521 1.15283C12.3624 1.05422 12.1795 1.00195 11.9928 1.00195C11.8061 1.00195 11.6231 1.05422 11.4646 1.15283C11.306 1.25144 11.1783 1.39246 11.0958 1.55992L8.24277 7.33992Z"
      fill="#002B21"
    />
  </svg>
);
